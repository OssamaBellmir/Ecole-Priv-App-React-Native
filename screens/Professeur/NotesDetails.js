...
function NotesPereDetails() {
  ...
  const [noteType, setNoteType] = useState('note1'); // 'note1' ou 'note2'
    ...
    const fetchSubject = async () => {
      try {
        ...
        if (subjectSnapshot.exists()) {
          const subjectData = { id: subjectSnapshot.id, ...subjectSnapshot.data() };
          console.log("Nom de la matière :", subjectData.name); // Afficher le nom de la matière dans la console
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {subject && console.log("Nom de la matière dans le rendu :", subject.name)} {/* Afficher le nom de la matière dans la console */}
