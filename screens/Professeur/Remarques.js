...
function Remarques({ route }) {
        ...
        classQuerySnapshot.forEach((doc) => {
          ...
          classPromises.push((async () => {
            ...
            if (isFilteredClass) {
              ...
              // Fetch unread remarks count for each subject
              await fetchUnreadRemarksCount(userId, classId, profId);
            }
          })());
        });
  ...
  const fetchUnreadRemarksCount = async (userId, classId, profId) => {
    try {
      const remarksRef = collection(firestore, `Classes/${userId}/ClassesIds/${classId}/Remarques`);
      const remarksSnapshot = await getDocs(remarksRef);
      const counts = {};
      remarksSnapshot.forEach((doc) => {
        const data = doc.data();
        if (data.profId === profId && data.status === "Non Lu") {
      ...
      setUnreadRemarksCounts(prevCounts => ({ ...prevCounts, ...counts }));
    } catch (error) {
  ...
  const navigateToNotesDetails = (userId, classId, profId, matiereId, student) => {
    navigation.navigate('RemarquesDetails', { userId, classId, profId, matiereId, student });
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        {classData.map(({ classId, classTitle }, index) => (
          <TouchableOpacity
            key={index}
            style={[styles.classButton, selectedClassId === classId && styles.selectedClassButton]}
            onPress={() => handleClassSelection(classId)}
          >
            <Text style={styles.buttonText}>{classTitle}</Text>
            {profsMatieresData
              .filter((data) => data.classId === classId)
              .map((data, idx) => (
                <View key={idx} style={styles.subjectContainer}>
                  <Text style={styles.subjectText}>{data.matiere}</Text>
                  {unreadRemarksCounts[data.matiereId] > 0 && (
                    <Badge style={styles.badge}>{unreadRemarksCounts[data.matiereId]}</Badge>
                  )}
                </View>
              ))}
          </TouchableOpacity>
